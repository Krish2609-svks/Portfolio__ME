...
export default function CertificationsPage() {
  ...
  const handleSave = async (item: Certification) => {
    ...
    if (isNew) {
      const { data } = await supabase.from('certifications').insert([dataToSave]).select().single();
      if (data) {
        setItems(items.map(i => i.id === item.id ? data : i));
      }
    } else {
      await supabase.from('certifications').update(dataToSave).eq('id', item.id);
    }
    ...
    <div className="h-full flex flex-col p-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-zinc-400">Manage professional certifications and courses.</p>
        </div>
                ...
                {editingId === item.id ? (
                  <div className="flex-1 grid grid-cols-2 gap-4">
                    <div className="col-span-full">
                      ...
                      <input type="text" value={item.name} onChange={e => setItems(items.map(x => x.id === item.id ? { ...x, name: e.target.value } : x))} className="w-full bg-zinc-900 border border-zinc-700 rounded-md px-3 py-1.5 text-sm text-white" />
                    </div>
                    <div>
                    ...
                    <div className="col-span-full">
                      <label className="block text-xs text-zinc-400 mb-1">Credential URL</label>
                      <input type="text" value={item.url || ""} onChange={e => setItems(items.map(x => x.id === item.id ? { ...x, url: e.target.value } : x))} className="w-full bg-zinc-900 border border-zinc-700 rounded-md px-3 py-1.5 text-sm text-white" />
                    </div>
                  ...
                  <div className="flex-1">
                    ...
                    {item.url && <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-500 hover:underline line-clamp-1">{item.url}</a>}
                  </div>
                )}
